...
Niniejsza praca inżynierska dotyczy analizy przebiegu intonacji w zdaniach wypowiedzianych w języku polskim. Omówione zostały zagadnienia związane z wytwarzaniem mowy, częstotliwością podstawową oraz funkcjami intonacji. Celem pracy było zaobserwowanie charakterystycznych cech, towarzyszących każdemu z rodzajów wypowiedzi. W ceu określenia właściwości intonacji sporządzono bazę 90 nagrań, zawierających zdania wypowiedziane przez mężczyzn oraz kobiety. Aby umożliwić proces analizy, zaprojektowany oraz zaimplementowany został program w języku C++.
Trajektorie częstotliwości podstawowej zostały uzyskane za pomocą algorytmu YIN. Zaproponowany został zbiór reguł służących klasyfikowaniu wypowiedzi. Wyniki klasyfikacji zostały porównane z wynikami uzyskanymi przez program, gdy trajektorie zostały uzyskane za pomocą programu Praat. 

  \vskip.5\baselineskip
 
 \centerline{\bfseries Abstract}
In this thesis, an analysis of intonation in utterances spoken in the Polish
language was conducted. Topics related to speech production, fundamental
frequency estimation and intonation mechanism were discussed. The purpose of
this work was to observe the unique properties of fundamental frequency
trajectories that are discriminative for several types of the spoken
utterance. In order to determine the properties of intonation a database
containing 90 recordings with statements spoken by male and female speakers
has been prepared. To facilitate an analysis process, a dedicated application
has been designed and implemented in C++ language. The fundamental frequency
trajectories were estimated using the YIN algorithm and a set of rules have
been proposed to classify the type of utterance. The classification results
were compared with the results obtained by the system where the trajectories
were estimated by Praat application, and the same intonation rules were
applied.
